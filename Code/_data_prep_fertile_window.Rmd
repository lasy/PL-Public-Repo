...
foreach(file = days_files)%do%{
  ...
  j = which(((days$variable == "bleeding") & (days$value == 0)) |
              ((days$variable == "sex") & (days$value == 0)) |
...
if(!file.exists(file_obs) | TRUE){
  
  input_folder = paste0(IO$output_data,"Days_long/")
  days_files = sample(list.files(input_folder),2)
  
  obs = foreach(file = days_files, .combine = rbind)%do%{
    cat(file, "\n")
    days = read_feather(paste0(input_folder,file))
    user_ids = sample(unique(days$user_id), 20)
...
load(file = paste0(IO$output_data,"unfitted_bleeding_cycles_model.Rdata"), verbose = TRUE)
