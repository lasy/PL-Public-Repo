...
hsmmfit <- function(x,model,mstep=NULL,M=NA,maxit=100,lock.transition=FALSE,lock.d=FALSE,graphical=FALSE, debug = FALSE) {
  ...
  for(it in 1:maxit) {
    ...
    if(any(colSums(state_wt)==0)) warning("Warning: At least one state has an expected number of occurences equal to 0.\n This may be caused by bad starting parameters or insufficent sample size\n")
    new.model.parms.emission = mstep(x,state_wt, old_parms_emission = new.model$parms.emission)
    new.model$parms.emission = new.model.parms.emission
    ...
    if(lock.d) {
      ...
      if(sojourn.distribution=="nonparametric") {
        ...
        for(i in 1:J) {  
          #cat(it, "\t", i , "\n")
          #if((it == 2) & (i == 15)){ browser ()}
          wt_gamma = new.model$d[,i]
          if(sum(wt_gamma)>0){
            tmp = gammafit(1:M,wt=wt_gamma)
...
predict.hsmm <- function(object,newdata,method="viterbi",...) {
  ...
  if(method=="viterbi") {
    ...
    loga = as.double(log(object$model$transition)) # log of transition probabilities
    loga[loga==-Inf]=m  # replacing -Inf by a very large negative value
    logstart = as.double(log(object$model$init)) # log of initial probabilities
    logstart[logstart==-Inf|is.nan(logstart)] = m # replacing -Inf by a very large negative value
    d = apply(object$model$d,2,function(x) x/sum(x))# d is the sojourn density
    D = apply(d,2,function(x) rev(cumsum(rev(x)))) # D is the inverse cummulative density function for the sojourn
    d = log(d)
    d[d==-Inf]=m
    D = log(D)
    D[D==-Inf]=m
