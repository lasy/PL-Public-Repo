...
mstep.mvnorm <- function(x, wt, old_parms_emission) {
  idx <-  apply(is.na(x),1,any) # Find rows with NA's (cov.wt does not like them)
  x  <- x[!idx,,drop=FALSE]
  wt <- wt[!idx,,drop=FALSE]
  #  print(class(x))
  if(!missing(old_parms_emission) & 
     (is.list(old_parms_emission)) & 
     all(names(old_parms_emission) == c("mu","sigma")) &
     (length(old_parms_emission$mu) == ncol(wt)) & 
     (length(old_parms_emission$sigma) == ncol(wt))){
    emission = old_parms_emission
  }else{emission <- list(mu = list(), sigma = list())}
